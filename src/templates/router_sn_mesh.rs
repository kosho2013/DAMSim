...
#[context_macro]
pub struct router_sn_mesh<A: Clone> {
...
impl<A: DAMType + num::Num> router_sn_mesh<A>
where
router_sn_mesh<A>: Context,
{
    pub fn new(
        ...
        let router_sn_mesh = router_sn_mesh {
            in_stream,
            in_dict,
            in_len,
            out_stream,
        ...
        for i in 0..in_len
        {
            ...
            router_sn_mesh.in_stream[idx].attach_receiver(&router_sn_mesh);
        }
        for i in 0..out_len
        {
            let idx: usize = i.try_into().unwrap();
            router_sn_mesh.out_stream[idx].attach_sender(&router_sn_mesh);
        }
...
impl<A: DAMType + num::Num> Context for router_sn_mesh<A> {
    fn run(&mut self) {
...
mod tests {
    ...
    use crate::templates::router_sn_mesh::router_sn_mesh;
    use crate::token_vec;

    #[test]
    fn test_router_sn_mesh() {
        ...
        let router_sn_mesh = router_sn_mesh::new(in_stream, in_dict, in_len, out_stream, out_dict, out_len, x_dim, y_dim, x, y, num_input, num_vc, dummy);
        parent.add_child(router_sn_mesh);
