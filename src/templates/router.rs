...
//         };
//         for i in 0..in_len * num_vc
//         {
//             let idx: usize = i.try_into().unwrap();
//             router.in_stream[idx].attach_receiver(&router);
//         }
//         for i in 0..out_len * num_vc
//         {
//             let idx: usize = i.try_into().unwrap();
...
//         let mut in_num_received = vec![]; // NSEWL
...
//         let mut out_num_sent = vec![]; // NSEWL
...
//             let mut data_vec = vec![]; // all VCs
//             let mut dst_x_vec = vec![]; // all VCs
//             let mut dst_y_vec = vec![]; // all VCs
...
//                 } else
//                 {   
//                     for j in 0..self.num_vc
//                     {
//                         let idx_stream = in_idx_vec[i] + j;

//                         if in_closed[i*self.num_vc+j]
//                         {
//                             data_vec.push(invalid);
//                             dst_x_vec.push(invalid);
//                             dst_y_vec.push(invalid);
//                         } else
//                         {
//                             let peek_result = self.in_stream[idx_stream].peek();
//                             match peek_result {
//                                 PeekResult::Something(_) =>
//                                 {
//                                     let data = self.in_stream[idx_stream].dequeue(&self.time).unwrap().data;
...
//                                     dst_x_vec.push(invalid);
//                                     dst_y_vec.push(invalid);
//                                     in_closed[i*self.num_vc+j] = true;
//                                 },
//                             }
//                         }
//                     }
...
//                 { 
//                     if dst_x_vec[i] == self.x && dst_y_vec[i] == self.y // exit local port, only send to VCID=0
//                     {
...
//                     cnt1 += 1;
//                 } else {
//                     if in_num_received[i] == in_num_received_limit[i] * self.num_input
//                     {
//                         cnt1 += 1;
//                     }
//                 }
...
//             if aaa <= 200 && self.x == 1 && self.y == 2
//             {
//                 println!("x:{}, y:{}, in_closed{:?}, data_vec{:?}, in_num_received{:?}, in_num_received_limit{:?}, out_num_sent{:?}, out_num_sent_limit{:?}, cnt1:{}, cnt2:{}, in_invalid{:?}, out_invalid{:?}   ", self.x, self.y, in_closed, data_vec, in_num_received, in_num_received_limit, out_num_sent, out_num_sent_limit, cnt1, cnt2, in_invalid, out_invalid);
//                 // println!("x:{}, y:{}, data_vec{:?}", self.x, self.y, data_vec);
//                 aaa += 1;
//             }
