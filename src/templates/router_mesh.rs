    ...
    fn run(&mut self) {
        ...
        loop
        {
            ...
            if lowest_idx != invalid
            {
                ...
                {
                    // println!("nothing x:{}, y:{}, lowest_idx:{}, advance to future_time:{}", self.x, self.y, lowest_idx, future_time[lowest_idx]);

                    self.time.advance(future_time_tmp[lowest_idx]);

                } else // Something
                {
                    // println!("something x:{}, y:{}, lowest_idx:{}", self.x, self.y, lowest_idx);
                    ...
                    if dst_x_vec[lowest_idx] == self.x && dst_y_vec[lowest_idx] == self.y // exit local port
                    {
                        if out_idx_vec[4] == invalid
                        {
                            ...
                            self.out_stream[out_idx_vec[4]].enqueue(&self.time, ChannelElement::new(self.time.tick()+1, data_vec[lowest_idx].clone())).unwrap();
                            out_num_sent[4] += 1;
                        }
