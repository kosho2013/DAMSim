...
impl<A: DAMType + num::Num> router_SEW<A>
where
router_SEW<A>: Context,
{
    pub fn new(
        in_S: Receiver<usize>,
        in_E: Receiver<usize>,
        ...
        router_SEW.in_S.attach_receiver(&router_SEW);
        router_SEW.in_E.attach_receiver(&router_SEW);
        router_SEW.in_W.attach_receiver(&router_SEW);
...
impl<A: DAMType + num::Num> Context for router_SEW<A> {
    fn run(&mut self) {
