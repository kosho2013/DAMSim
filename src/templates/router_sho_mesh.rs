...
#[context_macro]
pub struct router_sho_mesh<A: Clone> {
...
impl<A: DAMType + num::Num> router_sho_mesh<A>
where
router_sho_mesh<A>: Context,
{
    pub fn new(
        ...
        let router_sho_mesh = router_sho_mesh {
            in_stream,
            in_dict,
            in_len,
            out_stream,
        ...
        for i in 0..in_len
        {
            ...
            router_sho_mesh.in_stream[idx].attach_receiver(&router_sho_mesh);
        }
        for i in 0..out_len
        {
            let idx: usize = i.try_into().unwrap();
            router_sho_mesh.out_stream[idx].attach_sender(&router_sho_mesh);
        }
...
impl<A: DAMType + num::Num> Context for router_sho_mesh<A> {
    fn run(&mut self) {
