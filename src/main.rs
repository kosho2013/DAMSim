...
fn main() {
				...
				for j in 0..pcu_x.len()
				{
					if pcu_x[j] == x && pcu_y[j] == y
					{
						...
						} else if pcu_receiver_vec_tmp[0] == no_connection
						{
							...
							if topology_on_chip == "mesh"
							{
								let router_mesh = router_mesh::new(router_in_stream, router_in_dict, router_in_len, router_out_stream, router_out_dict, router_out_len, x_on_chip, y_on_chip, x, y, num_input_on_chip, max_counter, false, dummy);
								parent.add_child(router_mesh);
							} else if topology_on_chip == "torus"
							{
								let router_torus = router_torus::new(router_in_stream, router_in_dict, router_in_len, router_out_stream, router_out_dict, router_out_len, x_on_chip, y_on_chip, x, y, num_input_on_chip, max_counter, false, dummy);
								parent.add_child(router_torus);
							} else if topology_on_chip == "dragonfly"
							{
								let router_dragonfly = router_dragonfly::new(router_in_stream, router_in_dict, router_in_len, router_out_stream, router_out_dict, router_out_len, x_on_chip, y_on_chip, x, y, num_input_on_chip, max_counter, false, dummy);
						...
						} else if pcu_sender_vec_tmp[0] == no_connection
						{
							...
							if topology_on_chip == "mesh"
							{
								let router_mesh = router_mesh::new(router_in_stream, router_in_dict, router_in_len, router_out_stream, router_out_dict, router_out_len, x_on_chip, y_on_chip, x, y, num_input_on_chip, max_counter, true, dummy);
								parent.add_child(router_mesh);
							} else if topology_on_chip == "torus"
							{
								let router_torus = router_torus::new(router_in_stream, router_in_dict, router_in_len, router_out_stream, router_out_dict, router_out_len, x_on_chip, y_on_chip, x, y, num_input_on_chip, max_counter, true, dummy);
								parent.add_child(router_torus);
							} else if topology_on_chip == "dragonfly"
							{
								let router_dragonfly = router_dragonfly::new(router_in_stream, router_in_dict, router_in_len, router_out_stream, router_out_dict, router_out_len, x_on_chip, y_on_chip, x, y, num_input_on_chip, max_counter, true, dummy);
								parent.add_child(router_dragonfly);
							} else
							{
								panic!("Wrong!");
	...
	for i in 0..num_config
	{
		...
		for (key, value) in &my_dict
		{
			println!("used_link_map: {}, {:?}, {}", cnt, key, value);
			cnt += 1;
		}
		...
		for ele in all_routers
		{
			...
			if topology_off_chip == "mesh"
			{
				let router_mesh = router_mesh::new(router_in_stream, router_in_dict, router_in_len, router_out_stream, router_out_dict, router_out_len, x_off_chip, y_off_chip, x, y, num_input_off_chip, 1, true, dummy);
				parent.add_child(router_mesh);
			} else if topology_off_chip == "torus"
			{
				let router_torus = router_torus::new(router_in_stream, router_in_dict, router_in_len, router_out_stream, router_out_dict, router_out_len, x_off_chip, y_off_chip, x, y, num_input_off_chip, 1, true, dummy);
				parent.add_child(router_torus);
			} else if topology_off_chip == "dragonfly"
			{
				...
				let router_dragonfly = router_dragonfly::new(router_in_stream, router_in_dict, router_in_len, router_out_stream, router_out_dict, router_out_len, x_off_chip, y_off_chip, x, y, num_input_off_chip, 1, true, dummy);
				parent.add_child(router_dragonfly);
			} else
			{
				panic!("Wrong!");
